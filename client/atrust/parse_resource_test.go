...
func TestParseResourcePreservesAddrPretendForDomain(t *testing.T) {
	policy := []byte(`{"data":{"appList":{"data":{"appInfo":[{"apps":[{"ID":"tcp-app","NodeGroupID":"tcp-group","AccessModel":"L3VPN","addrPretend":true,"AddressList":[{"Protocol":"tcp","Port":"80","Host":"speedtest.xtu.edu.cn"}]}]}]}}}}`)
	c := &Client{}
	if err := c.parseResource(policy); err != nil {
		t.Fatalf("parseResource() error = %v", err)
	}

	resource, ok := client.MatchDomainResource(c.domainResources["speedtest.xtu.edu.cn"], "tcp", 80)
