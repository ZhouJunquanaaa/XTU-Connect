...
`)
	...
	options, _, err := loadStartupOptions([]string{
		...
		return []string{
			...
			"XTU_CONNECT_SERVER_PORT=555",
			"XTU_CONNECT_USERNAME=env-user",
		}
	})
	if err != nil {
		t.Fatal(err)
...
`)
	...
	options, _, err := loadStartupOptions([]string{"--auth-info"}, func() []string {
		return []string{"XTU_CONNECT_CONFIG=" + configFile}
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProtocolServerWorkaround(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "empty server falls back to default", protocol: "atrust", want: "vpn.xtu.edu.cn"},
		{name: "aTrust server remains unchanged", protocol: "atrust", server: "vpn.xtu.edu.cn", want: "vpn.xtu.edu.cn"},
		{name: "EasyConnect server remains unchanged", protocol: "easyconnect", server: "vpn.xtu.edu.cn", want: "vpn.xtu.edu.cn"},
...
func TestUnknownConfigurationKeyIsRejected(t *testing.T) {
	...
	if _, _, err := loadStartupOptions(nil, func() []string {
		return []string{"XTU_CONNECT_SERVER_ADRESS=typo.example.com"}
	}); err == nil {
...
func TestEnvironmentCollectionsUseCLISyntax(t *testing.T) {
	options, _, err := loadStartupOptions(nil, func() []string {
		return []string{
			"XTU_CONNECT_TCP_PORT_FORWARDING=127.0.0.1:2-10.0.0.2:2",
			"XTU_CONNECT_UDP_PORT_FORWARDING=127.0.0.1:3-10.0.0.3:3",
			"XTU_CONNECT_CUSTOM_PROXY_DOMAIN=one.example.com,two.example.com",
			"XTU_CONNECT_CUSTOM_DNS=host.example.com:192.0.2.1",
		}
	})
	if err != nil {
		t.Fatal(err)
...
func TestCLICollectionsOverrideEnvironmentCollections(t *testing.T) {
	options, _, err := loadStartupOptions([]string{
		...
		return []string{
			"XTU_CONNECT_TCP_PORT_FORWARDING=127.0.0.1:2-10.0.0.2:2",
			"XTU_CONNECT_UDP_PORT_FORWARDING=127.0.0.1:3-10.0.0.3:3",
			"XTU_CONNECT_CUSTOM_PROXY_DOMAIN=env.example.com",
		}
	})
	if err != nil {
		t.Fatal(err)
