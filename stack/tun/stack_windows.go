...
const interfaceName = "XTU Connect"
...
func NewStack(client client.Client, dnsHijack, fakeIP bool, ipResources []client.IPResource) (*Stack, error) {
	...
	command = exec.Command("netsh", "interface", "ipv4", "delete", "dnsservers", "XTU Connect", "all")
	err = command.Run()
	if err != nil {
		log.Printf("Run %s failed: %v", command.String(), err)
	}
	if dnsHijack {
		...
		command = exec.Command("netsh", "interface", "ipv4", "add", "dnsservers", "XTU Connect", dnsServerIP)
		err = command.Run()
		if err != nil {
			log.Printf("Run %s failed: %v", command.String(), err)
		}
	...
	hook_func.RegisterTerminalFunc("Close Tun Device", func(ctx context.Context) error {
		dev.Close()
		closeCommand := exec.Command("netsh", "interface", "ipv4", "delete", "dnsservers", "XTU Connect", "all")
