...
RUN --mount=target=. \
    ...
    go build -v -o /app/xtu-connect -trimpath \
    -ldflags "-s -w -buildid= -X main.xtuConnectVersion=${VERSION} -X main.CommitID=${COMMIT_ID}" .

# Import the binary from build stage
# use root container, but still use /home/nonroot to keep backward support
FROM gcr.io/distroless/static as prd
WORKDIR /home/nonroot
COPY --from=build /app/xtu-connect /home/nonroot
ENTRYPOINT ["/home/nonroot/xtu-connect" ,"-config", "/home/nonroot/config.toml"]
